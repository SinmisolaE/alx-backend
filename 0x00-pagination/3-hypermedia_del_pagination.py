...
class Server:
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        """ should return a dictionary with the following key-value pairs:
        ...
        current_idx = index
        data = []
        while len(data) < page_size and current_idx < len(dataset):
            if current_idx in dataset:
                data.append(dataset[current_idx])
            current_idx += 1

        next_index = current_idx if current_idx < len(dataset) else None
        return {
